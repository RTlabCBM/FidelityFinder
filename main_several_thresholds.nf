...
/* 
	...
	DESCRIPTION
	FidelityFinder is a bioinformatics analysis pipeline to determine the fidelity of transcriptases and the fidelity of DNA synthesis by 
...
process quality_filtering_and_fastq_to_fasta {
    ...
    output:
	path "${sampleID}.lengths_filtered.fastq"
	...
	# Filter assembled sequences by length (params.insert_length +- params.length_tolerance nt) and by quality
		awk 'BEGIN {FS = "\\t" ; OFS = "\\n"} {header = \$0 ; getline seq ; getline qheader ; getline qseq ; if (length(seq) >= ("${params.insert_length}"-"${params.length_tolerance}") && length(seq) <= ("${params.insert_length}"+"${params.length_tolerance}")) {print header, seq, qheader, qseq}}' "${assembled_reads_fastq}" > "${sampleID}.lengths_filtered.fastq"
		fastq_quality_filter -v -q "${params.minimum_quality_score}" -p "${params.minimum_percent_quality_bases}" -i "${sampleID}.lengths_filtered.fastq" -o "${sampleID}_lengths_and_qual_filtered.fastq" | tee "${sampleID}_fastq_quality_filter.log"
...
process consensus_construction {
	...
	tuple val("${sampleID}_t100"), path("${sampleID}_t100_consensus.fna"), env(max_barcode), val ("100"), emit: consensus_reads_t100_ch
	tuple val("${sampleID}_t90"), path("${sampleID}_t90_consensus.fna"), env(max_barcode), val ("90"), emit: consensus_reads_t90_ch
	tuple val("${sampleID}_t80"), path("${sampleID}_t80_consensus.fna"), env(max_barcode), val ("80"), emit: consensus_reads_t80_ch
	tuple val("${sampleID}_t75"), path("${sampleID}_t75_consensus.fna"), env(max_barcode), val ("75"), emit: consensus_reads_t75_ch
	tuple val("${sampleID}_t70"), path("${sampleID}_t70_consensus.fna"), env(max_barcode), val ("70"), emit: consensus_reads_t70_ch
	tuple val("${sampleID}_t60"), path("${sampleID}_t60_consensus.fna"), env(max_barcode), val ("60"), emit: consensus_reads_t60_ch
	tuple val("${sampleID}_t50"), path("${sampleID}_t50_consensus.fna"), env(max_barcode), val ("50"), emit: consensus_reads_t50_ch
	tuple val("${sampleID}_t0"), path("${sampleID}_t0_consensus.fna"), env(max_barcode), val ("0"), emit: consensus_reads_t0_ch

	
	script:
    """
	# Run python script to obtain consensus sequences by barcode
		python ${baseDir}/bin/consensus_construction.py --fasta_file "${fasta_file}" --reference "${reference_seq_fasta}" --output_prefix "${sampleID}" --inferior ${params.cutoff} --threshold_consensus_list "0,50,60,70,75,80,90,100" -c $task.cpus | tee "${sampleID}_consensus_construction.log"
...
process consensus_sequences_mapping {
	...
	input:
	tuple val(sampleID), path(consensus_fasta), val(max_barcode), val(threshold_value)
	path reference_seq_fasta
	 
	output:
	path "${sampleID}_align.sam"
	tuple val(sampleID), path("${sampleID}_align_sort.bam"), val(max_barcode), val(threshold_value), env(consensus_number), emit: bam_file_ch
...
process variant_calling {
	...
	input:
	tuple val(sampleID), path(bam_file), val(max_barcode), val(threshold_value), val(consensus_number)
	path reference_seq_fasta
	
	output:
	tuple val(sampleID), path("${sampleID}.vcf"), val(max_barcode), val(threshold_value), val(consensus_number),  emit: vcf_ch
...
process vcf_analysis {
	...
	input:
	tuple val(sampleID), path(vcf_file), val(max_barcode), val(threshold_value), val(consensus_number)
	val params.min_pos
	val params.max_pos
	...
	script:
    """
	python ${baseDir}/bin/vcf_analyzer.py --input_file "${vcf_file}" --consensus_number "${consensus_number}" --output_prefix "${sampleID}" --min_pos "${params.min_pos}" --max_pos "${params.max_pos}" --max_barcode "${max_barcode}" --cutoff ${params.cutoff} --threshold ${threshold_value}
	"""
}
...
workflow {
	...
	consensus_construction(quality_filtering_and_fastq_to_fasta.out.fasta_file_ch, ref_seq_ch, params.cutoff)
	
	//Step 6: Map consensus sequences
	consensus_sequences_mapping(consensus_construction.out.consensus_reads_t100_ch.concat(consensus_construction.out.consensus_reads_t90_ch, consensus_construction.out.consensus_reads_t80_ch, consensus_construction.out.consensus_reads_t75_ch, consensus_construction.out.consensus_reads_t70_ch, consensus_construction.out.consensus_reads_t60_ch, consensus_construction.out.consensus_reads_t50_ch, consensus_construction.out.consensus_reads_t0_ch), ref_seq_ch)
